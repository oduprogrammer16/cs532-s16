...
def user_correlation(userId,data,result,pref_list): 
...
if __name__ == '__main__':
	#ataList,userList,itemList = read_data_files(dataName='data_files/u.data',itemName='data_files/u.item',userName='data_files/u.user')
	data = Data_Set()
	...
	if '-h' in arguments:
		...
		for userId in range(1,len(sys.argv)):
			...
			if len(list(filter(lambda user: user['user_id'] == int(sys.argv[userId]),data.critic_list))) != 0:
				...
				userThreads.append(threading.Thread(name=str(sys.argv[userId]),target=user_correlation,args=(int(sys.argv[userId]),data,userThreadResults,prefs)))
			else:
